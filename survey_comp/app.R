...
ui <- fluidPage(
    ...
    conditionalPanel(condition = "input.disclaimer == 0",
                     br(),
                     ...
                     br(),
                     p("The following application was created to collect survey responses from a targeted audience. As you work through the 
                     survey, please feel free to answer the questions.", strong("Your provided responses will be stored. However, they will be deleted periodically and will not be used in research."), "Thank you!"),
                     br(),
                     actionButton("disclaimer","Continue")
    ),
    
    conditionalPanel(condition="input.agr==0 && input.disclaimer == 1",
                     br(),
                     h1("Assessing Normality Using Q-Q Plots"),
                     br(),
    ...
    conditionalPanel(condition = "input.submit == 1",
                     ...
                     uiOutput("link"),
                     p("(Link has been deactivated for this demonstration.)")
    )
)
...
server <- function(input, output, session) {
    ...
    myIndicators <- reactive({
        ...
        userKeyLocation <- gs4_get('https://docs.google.com/spreadsheets/d/1NR7dbjaEFPU1jJaqssn5k1RJZsIj7rEttVCMpBZR1xQ/edit#gid=1629775005')
        sheet <- read_sheet(userKeyLocation, sheet = "survey_order")
        userID <- length(sheet$userID) +1
        
        readIn <- gs4_get('https://docs.google.com/spreadsheets/d/1NR7dbjaEFPU1jJaqssn5k1RJZsIj7rEttVCMpBZR1xQ/edit#gid=1629775005')
    ...
    output$submission <- renderText({
        ...
        if(input$submit[1] == 0){
            ...
            # submit score
            ss2 <- gs4_get('https://docs.google.com/spreadsheets/d/1NR7dbjaEFPU1jJaqssn5k1RJZsIj7rEttVCMpBZR1xQ/edit#gid=1629775005')
    ...
    output$incentiveKey <- renderText({
        ...
        keyLocation <- gs4_get('https://docs.google.com/spreadsheets/d/1muizTy8l-ACoe8QTK9DYdgJLhd4cMONLyRlR8l5P7Ig/edit#gid=0')
